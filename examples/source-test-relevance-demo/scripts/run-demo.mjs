...
const baselineFiles = {
  ...
  "tests/refund-service.test.ts": "import { strict as assert } from \"node:assert\";\n\nfunction refundOrder() {\n  return { ok: true };\n}\n\nassert.equal(refundOrder().ok, true);\n",
  "tests/invoice-service.test.ts": "export const invoiceCoverage = true;\n"
};
const extraPathsToReset = [evidencePath, contractPath];

function loadScenario(name) {
  return JSON.parse(fs.readFileSync(path.join(exampleRoot, "scenarios", `${name}.json`), "utf8"));
}
...
function stagePlugin() {
  copyDirectoryRecursive(
    ...
    path.join(exampleRoot, "node_modules", "@agent-guardrails", "plugin-ts")
  );
}
    ...
    captureLogs(() =>
      runPlan({
        ...
        flags: {
          ...
          "expected-test-targets": (scenario.expectedTestTargets ?? []).join(","),
...
function printScenarioSummary(result, scenario) {
  ...
  assert.equal(
    findingCodes.includes("source-test-relevance-missed-expected-targets"),
    false,
    `Scenario ${scenario.name} should clear the source-test relevance error.`
  );
  assert.equal(
    findingCodes.includes("source-test-relevance-weak"),
    false,
    `Scenario ${scenario.name} should clear the source-test relevance warning.`
  );
  console.log("- Proof signal: no source-test relevance finding remains");
}
