...
const baselineFiles = {
  "src/orders/refund-workflow.ts": "function buildRefundPayload() {\n  return { ok: true, visibility: \"internal\" };\n}\n\nfunction refundOrder() {\n  return buildRefundPayload();\n}\n",
  "tests/refund-workflow.test.ts": "export const refundWorkflowBaseline = true;\n"
};
...
function printScenarioSummary(result, scenario) {
  ...
  assert.equal(
    findingCodes.includes("interface-drift-implementation-only"),
    false,
    `Scenario ${scenario.name} should clear the interface drift finding.`
  );
  console.log("- Proof signal: no interface drift finding remains");
}
