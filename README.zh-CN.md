...
- 🔍 **漂移检测** — 捕获并行抽象、接口变更
- 🛡 **受保护路径** — 关键文件保持不被触碰
- 🔒 **安全卫生** — 警告硬编码密钥、不安全模式、敏感文件变更
- 📊 **信任评分** — 0-100 复合评分，带分级判定
- 🔧 **自动修复** — 一级问题自动修复，零副作用
...
Pro 当前通过私有付费包渠道分发。获得访问权限后，在 `.agent-guardrails/config.json` 中添加 Pro license key：
...
仅此而已 - 当 Pro 包和 license 都可用时，下次运行 `check` 会自动激活 Pro 功能。可运行 `agent-guardrails pro status` 验证安装、license 状态，以及下一步应该执行的 Pro 动作。
