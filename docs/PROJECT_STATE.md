...
Last updated: 2026-04-13 (post-v0.19.3, OSS Pro learned proof rendering)
...
**v0.19.3** - Published OSS release. Post-v0.19.3 work in progress: `check --json` now exposes installed Pro proof plans at the top level, and `pro status` plus `check --review` render Pro proof-memory, learned proof, and prioritized proof-surface wording when available.
