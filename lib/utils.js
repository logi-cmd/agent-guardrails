...
export function readConfig(repoRoot) {
  ...
  let parsed;
  try {
    parsed = JSON.parse(readJsonText(configPath));
  } catch (cause) {
    throw new Error(
...
export function readTaskContract(repoRoot, customPath = defaultTaskContractPath) {
  ...
  let contract;
  try {
    contract = JSON.parse(readJsonText(contractPath));
  } catch (cause) {
    throw new Error(
...
function listFilesRecursively(rootDir, currentDir = rootDir) {
  ...
  return entries.flatMap((entry) => {
    ...
    if (entry.isDirectory()) {
      return listFilesRecursively(rootDir, entryPath);
    }
    if (!entry.isFile()) {
      return [];
    }
    return path.relative(rootDir, entryPath).replaceAll("\\", "/");
  });
}

function expandChangedPath(gitRoot, filePath) {
  const absolutePath = path.join(gitRoot, filePath);
  if (!filePath.endsWith("/") || !fs.existsSync(absolutePath) || !fs.statSync(absolutePath).isDirectory()) {
    return [filePath];
  }

  return listFilesRecursively(absolutePath).map((nestedPath) => `${filePath}${nestedPath}`);
}
...
function parseGitDiffNameStatusZ(output) {
  ...
  for (let index = 0; index < entries.length;) {
    ...
    if ((code === "R" || code === "C") && entries[index] && entries[index + 1]) {
      files.push(entries[index], entries[index + 1]);
      index += 2;
      continue;
    }
...
function sameDirectory(leftPath, rightPath) {
  try {
    ...
    // Fall through to string comparison when stat is unavailable.
  }

  const leftReal = realPathForComparison(leftPath);
  const rightReal = realPathForComparison(rightPath);
  const normalizedLeft = process.platform === "win32" ? leftReal.toLowerCase() : leftReal;
  const normalizedRight = process.platform === "win32" ? rightReal.toLowerCase() : rightReal;
  return normalizedLeft === normalizedRight;
}

function repoPrefix(gitRoot, repoRoot) {
  if (sameDirectory(gitRoot, repoRoot)) {
    return "";
  }
  ...
  while (true) {
    ...
    if (sameDirectory(parent, gitRoot)) {
      return segments.join("/");
    }
    current = parent;
  }

  return path.relative(realPathForComparison(gitRoot), realPathForComparison(repoRoot)).replaceAll("\\", "/");
}
...
export function listChangedFiles(repoRoot) {
  ...
  try {
    ...
    const prefix = repoPrefix(gitRoot, repoRoot);

    return {
      files: parseGitStatusPorcelainZ(output)
        .flatMap((filePath) => expandChangedPath(gitRoot, filePath))
...
export function listChangedFilesFromBaseRef(repoRoot, baseRef) {
  ...
  try {
    const output = execFileSync(
      "git",
      ["diff", "--name-status", "-z", "--diff-filter=ACMRD", `${baseRef}...HEAD`],
...
export function getParentScope(filePath) {
