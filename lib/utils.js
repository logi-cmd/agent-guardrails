...
function repoPrefix(gitRoot, repoRoot) {
  const resolvedGitRoot = realPathForComparison(gitRoot);
  const resolvedRepoRoot = realPathForComparison(repoRoot);
  const comparableGitRoot = process.platform === "win32" ? resolvedGitRoot.toLowerCase() : resolvedGitRoot;
  const comparableRepoRoot = process.platform === "win32" ? resolvedRepoRoot.toLowerCase() : resolvedRepoRoot;
  if (comparableGitRoot === comparableRepoRoot) {
    return "";
  }
  const prefix = path.relative(resolvedGitRoot, resolvedRepoRoot);
  if (!prefix || prefix === ".") {
    return "";
  }
