...
function evaluateScenario(result, scenario) {
  ...
  if (scenario.expected.verdict) {
    assert.equal(
      result.verdict,
      scenario.expected.verdict,
      `Scenario "${scenario.name}" produced an unexpected verdict.`
    );
  }

  if (scenario.expected.deployReadinessStatus) {
    assert.equal(
      result.deployReadiness?.status,
      scenario.expected.deployReadinessStatus,
      `Scenario "${scenario.name}" produced an unexpected deploy-readiness status.`
    );
  }
...
export async function runActiveScenario(repoRoot, scenario) {
  ...
  try {
    ...
    return {
      ...
      deployReadinessStatus: checkResult.result.deployReadiness?.status ?? "unknown",
