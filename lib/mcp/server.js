...
async function callTool(name, args, defaultRepoRoot) {
  ...
  if (name === "plan_rough_intent") {
    ...
    if (mode === "auto" && parsed.confidence >= 0.6) {
      ...
      return createJsonResult({
        ...
        message: locale === "zh-CN"
          ? `自动接受建议（置信度 ${(parsed.confidence * 100).toFixed(0)}% >= 60%）`
          : `Auto-accepted suggestion (confidence ${(parsed.confidence * 100).toFixed(0)}% >= 60%)`
      });
    }
    ...
    return createJsonResult({
      ...
      message: locale === "zh-CN"
        ? `检测到 ${parsed.taskType} 类型任务，置信度 ${(parsed.confidence * 100).toFixed(0)}%。请确认或修改。`
        : `Detected ${parsed.taskType} task with ${(parsed.confidence * 100).toFixed(0)}% confidence. Please confirm or modify.`
    });
  }
