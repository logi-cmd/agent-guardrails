...
import { loadPro } from "../pro/status.js";
...
const LOOP_ORIENTED_TOOLS = new Set([
...
const DEFAULT_SESSION_CALL_LIMIT = 50;
...
async function getProMcpIntegration(repoRoot) {
  const pro = await loadPro(repoRoot);
  if (!pro?.getProMcpToolDefinitions) {
    return { pro: null, tools: [] };
  }

  const definitions = await pro.getProMcpToolDefinitions({ repoRoot });
  const ossToolNames = new Set(TOOL_DEFINITIONS.map((tool) => tool.name));
  const tools = (Array.isArray(definitions) ? definitions : [])
    .map(normalizeToolDefinition)
    .filter((tool) => tool && !ossToolNames.has(tool.name));

  return { pro, tools };
}

async function getAvailableToolDefinitions(repoRoot) {
  const { tools } = await getProMcpIntegration(repoRoot);
  return [...TOOL_DEFINITIONS, ...tools];
}

async function callProToolIfAvailable(name, args = {}, defaultRepoRoot = process.cwd()) {
  const repoRoot = args.repoRoot || defaultRepoRoot;
  const { pro, tools } = await getProMcpIntegration(repoRoot);
  if (!tools.some((tool) => tool.name === name)) {
    return null;
  }
  if (!pro?.callProMcpTool) {
    throw createError(-32601, `Pro MCP tool "${name}" is listed but the installed Pro package cannot run it.`);
  }

  const data = await pro.callProMcpTool(name, {
    ...args,
    repoRoot
  });
...
async function callTool(name, args, defaultRepoRoot) {
  ...
  if (name === "start_agent_native_loop") {
    ...
    if (!result || result.error) {
      throw createError(-32010, result?.message || "Missing .agent-guardrails/config.json in the requested repo.");
    }
  ...
  if (name === "check_after_edit") {
    ...
    if (freshCache && cacheData) {
      ...
      const scoreVerdict = result?.scoreVerdict;
      const errors = findings.filter(f => f && (f.severity === "error" || f.severity === "high"));
      const warnings = findings.filter(f => f && f.severity === "warning");
      const newFindings = findings.map(f => ({
        severity: f.severity || "info",
      ...
      if (errors.length > 0) {
        ...
        if (score !== undefined && score !== null) {
          ...
          lns.push(`📊 Trust Score: ${"█".repeat(filled)}${"░".repeat(empty)} ${score}/100 (${scoreVerdict || "unknown"})`);
        }
        lns.push("❌ Errors:");
        for (const e of errors.slice(0, 5)) {
          lns.push(`  - ${e.rule || e.id || "unknown"}: ${e.message || e.description || ""}`);
        ...
        data = { status: "issues", newFindings, score, scoreVerdict, cacheAge: null };
        humanSummary = lns.join("\n");
      } else {
        const lns = [];
  ...
  if (name === "finish_agent_native_loop") {
    ...
    if (!result || result.error) {
      throw createError(-32010, result?.message || "Missing task contract for the requested repo.");
    }
  ...
  const proResult = await callProToolIfAvailable(name, args, defaultRepoRoot);
  if (proResult) {
    return proResult;
  }
...
export { callTool, LOOP_ORIENTED_TOOLS, DEFAULT_SESSION_CALL_LIMIT };
...
async function handleMessage(message, context) {
  ...
  if (message.method === "tools/list") {
    return {
      ...
      id: message.id,
      result: {
        tools: await getAvailableToolDefinitions(context.repoRoot)
      }
    };
  }
  ...
  if (message.method === "tools/call") {
    ...
    if (LOOP_ORIENTED_TOOLS.has(name)) {
      context._loopCallCount = (context._loopCallCount || 0) + 1;
      if (context._loopCallCount > context._loopCallLimit) {
        return {
          jsonrpc: "2.0",
          id: message.id,
          error: {
            code: -32020,
            message: `Session call limit exceeded: ${context._loopCallCount} calls to loop-oriented tools (limit: ${context._loopCallLimit}). Restart the MCP server to reset.`,
            data: {
              limit: context._loopCallLimit,
              current: context._loopCallCount,
              tool: name
            }
          }
        };
...
export async function startMcpServer({
  ...
  sessionCallLimit = DEFAULT_SESSION_CALL_LIMIT
} = {}) {
  const context = { repoRoot, _loopCallLimit: sessionCallLimit, _loopCallCount: 0 };
