...
import { selectCheckRuntime } from "../rust-runtime.js";
import { getSetupAgentDefinition } from "../setup/agents.js";
import { projectRoot, readConfig, resolveRepoRoot, supportedAdapters } from "../utils.js";
...
function checkRuntimeSelection() {
  try {
    const runtime = selectCheckRuntime({ root: projectRoot });
    if (runtime.kind === "rust") {
      return {
        key: "checkRuntime",
        passed: true,
        detail: `Check runtime: Rust (${runtime.reason}).`
      };
    }
    return {
      key: "checkRuntime",
      passed: true,
      detail: `Check runtime: Node (${runtime.reason}). Set AGENT_GUARDRAILS_RUNTIME=rust to force Rust, or ship native/* to enable Rust by default.`
    };
  } catch (error) {
    return {
      key: "checkRuntime",
