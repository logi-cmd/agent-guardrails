...
async function printTextResult(result, t, { reviewMode = false } = {}) {
  ...
  if (result.failures.length > 0) {
    console.log(`❌ ${result.failures.length} ${resolvedLocale === "zh-CN" ? "个错误需要修复" : "error(s) to fix"} — ${result.failures.length === 1
      ? (resolvedLocale === "zh-CN" ? "修复后重新运行检查" : "fix it and re-run check")
      : (resolvedLocale === "zh-CN" ? "修复后重新运行检查" : "fix them and re-run check")
    }`);
  } else if (result.score != null && result.score < 70) {
    console.log(`⚠️ ${resolvedLocale === "zh-CN" ? `评分: ${result.score}/100` : `Score: ${result.score}/100`} — ${resolvedLocale === "zh-CN" ? "需要关注" : "needs attention"} — ${resolvedLocale === "zh-CN" ? "请查看上方发现项" : "review findings above"}`);
  } else if (result.warnings.length > 0) {
    console.log(`⚡ ${resolvedLocale === "zh-CN" ? `评分: ${result.score ?? "N/A"}/100` : `Score: ${result.score ?? "N/A"}/100`} — ${result.warnings.length} ${resolvedLocale === "zh-CN" ? "个警告" : "warning(s)"} — ${resolvedLocale === "zh-CN" ? "建议处理后再合并" : "recommended to address before merge"}`);
  } else {
    console.log(`✅ ${resolvedLocale === "zh-CN" ? `评分: ${result.score ?? "N/A"}/100` : `Score: ${result.score ?? "N/A"}/100`} — ${resolvedLocale === "zh-CN" ? "可以合并" : "ready to merge"}`);
  }

  if (result.review?.pro?.categoryScores) {
    const header = resolvedLocale === "zh-CN"
      ? "\n─── 分类评分 ─────────────────────────────────"
      : "\n─── Category Breakdown ────────────────────────────";
    console.log(header);
    try {
      const { formatProCategoryBreakdown } = await import("../check/pro/index.js");
      const lines = formatProCategoryBreakdown(result.review.pro.categoryScores, resolvedLocale);
      for (const line of lines) {
        console.log(line);
      }
    } catch { /* Pro not available */ }
  }

  // ─── Quick Counts ─────────────────────────────────
  const parts = [];
  if (result.counts.changedFiles > 0) parts.push(`${result.counts.changedFiles} ${resolvedLocale === "zh-CN" ? "个文件" : "files"}`);
  if (result.counts.outOfScopeFiles > 0) parts.push(`${result.counts.outOfScopeFiles} ${resolvedLocale === "zh-CN" ? "个越界" : "out-of-scope"}`);
  if (result.counts.missingRequiredCommands > 0) parts.push(`${result.counts.missingRequiredCommands} ${resolvedLocale === "zh-CN" ? "个缺失命令" : "missing commands"}`);
  if (result.counts.warnings > 0) parts.push(`${result.counts.warnings} ${resolvedLocale === "zh-CN" ? "个警告" : "warnings"}`);
  if (result.counts.failures > 0) parts.push(`${result.counts.failures} ${resolvedLocale === "zh-CN" ? "个错误" : "errors"}`);
  if (parts.length > 0) {
  ...
  // ─── Details ──────────────────────────────────────
  console.log(`\n${resolvedLocale === "zh-CN" ? "─── 详情 ──────────────────────────────────" : "─── Details ──────────────────────────────────"}`);
  ...
  if (result.failures.length === 0 && result.warnings.length === 0) {
    console.log(`\n✅ ${resolvedLocale === "zh-CN" ? "所有检查通过" : "All checks passed."}`);
  } else if (result.failures.length === 0 && result.warnings.length > 0) {
    console.log(`\n⚡ ${result.warnings.length} ${resolvedLocale === "zh-CN" ? "个警告（未阻塞）" : "warning(s) (not blocking)"} — ${resolvedLocale === "zh-CN" ? "建议处理后再合并" : "recommended to address before merging"}`);
  }
}
...
export async function executeCheck({ repoRoot = process.cwd(), flags, locale = null, suppressExitCode = false } = { flags: {} }) {
  ...
  let enrichedReview = baseReview;
  try {
    const { enrichReview } = await import("../check/pro/index.js");
    enrichedReview = enrichReview(baseReview, { ...config, repoRoot }, context);
  } catch { /* Pro not available */ }
  ...
  try {
    const { getProNextActions } = await import("../check/pro/index.js");
    const proActions = getProNextActions(enrichedReview, { ...context, config: { ...config, repoRoot } });
    if (proActions.length > 0) {
      result.runtime.nextActions = [...(result.runtime.nextActions || []), ...proActions];
    }
  } catch { /* Pro not available */ }
