...
const RUNTIME_INTERNAL_FILES = new Set([
...
function printGoLiveDecision(review) {
  ...
  const report = review?.goLiveReport;
  if (report?.state === "ready") {
    const command = report.nextAction?.command;
    if (command) {
      console.log(`- Report next action: ${command}`);
    }
    console.log("- Pro report: run `agent-guardrails pro report` for the full go-live report.");
  }
}

function shouldUseAsciiOutput(flags = {}, locale = "en") {
  if (process.env.AGENT_GUARDRAILS_UNICODE === "1") {
    return false;
  }
  if (flags?.ascii || process.env.AGENT_GUARDRAILS_ASCII === "1") {
    return true;
  }
  return process.platform === "win32" && locale === "en";
}

function toAsciiOutput(value) {
  return String(value)
    .replaceAll("✅", "[ok]")
    .replaceAll("❌", "[x]")
    .replaceAll("⚠️", "[!]")
...
async function withAsciiConsole(enabled, callback) {
  if (!enabled) {
    return callback();
  }

  const originalLog = console.log;
  console.log = (...args) => originalLog(...args.map((arg) => toAsciiOutput(arg)));
  try {
...
function formatLearnedProofScore(learnedEvidence = {}) {
  ...
  if (learnedEvidence.memoryHealthPenalty != null && learnedEvidence.memoryHealthPenalty > 0) {
    parts.push(`memory health penalty ${learnedEvidence.memoryHealthPenalty}`);
  }
...
function hasGoLiveApproval(result) {
  return String(result?.goLiveDecision?.verdict || result?.review?.goLiveDecision?.verdict || "").toLowerCase() === "go";
}

function printDeploySurface(result, t) {
  if (result.deployReadiness && !hasGoLiveApproval(result)) {
...
function printProDeepSignals(review) {
  ...
  if (cheapestProof?.learnedEvidence?.summary) {
    ...
    if (cheapestProof.learnedEvidence.memoryHealthWarning) {
      console.log(`- Learned proof warning: ${cheapestProof.learnedEvidence.memoryHealthWarning}`);
    }
...
function normalizeCommandEvidence(command) {
  return String(command ?? "")
    .trim()
    .replace(/\s+/g, " ")
    .replace(/^((?:npx|npm|pnpm|yarn|node|bun))\.cmd\b/i, "$1");
}
...
export async function executeCheck({ repoRoot = process.cwd(), flags, locale = null, suppressExitCode = false } = { flags: {} }) {
  ...
  const changedFiles = diffResult.files
    ...
    .filter((filePath) => !RUNTIME_INTERNAL_FILES.has(filePath));
  ...
  const declaredEvidenceFiles = new Set(evidencePaths.map((evidencePath) => normalizeRepoPath(evidencePath)));
  const scopeRelevantChangedFiles = changedFiles.filter((filePath) => !declaredEvidenceFiles.has(filePath));
  const allowedChangeTypes = (taskContract?.allowedChangeTypes ?? [])
    .map((item) => normalizeChangeType(item))
    .filter(Boolean);
  const outOfTaskScopeFiles = findOutOfScopeFiles(scopeRelevantChangedFiles, taskAllowedPaths, repoRoot);
  const outOfIntendedFiles = findOutOfScopeFiles(scopeRelevantChangedFiles, intendedFiles, repoRoot);
  const requiredCommands = taskContract?.requiredCommands ?? [];
  const normalizedCommandsRun = new Set(commandsRun.map((command) => normalizeCommandEvidence(command)));
  const missingRequiredCommands = requiredCommands.filter((command) => {
    return !normalizedCommandsRun.has(normalizeCommandEvidence(command));
  });
  ...
  const outOfScopeFiles = findOutOfScopeFiles(scopeRelevantChangedFiles, allowedPaths, repoRoot);
...
export async function runCheck({ flags, locale = null } = { flags: {} }) {
  ...
  const resolvedLocale = t("check.title") === "Agent Guardrails 检查结果" ? "zh-CN" : "en";
  await withAsciiConsole(shouldUseAsciiOutput(flags, resolvedLocale), () => printTextResult(result, t, { reviewMode }));
  return result;
}
