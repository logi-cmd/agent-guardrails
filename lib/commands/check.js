...
import { createFindingStore, addFinding } from "../check/finding.js";
...
function printTextResult(result, t, { reviewMode = false } = {}) {
  ...
  if (result.verdict) {
    console.log(`- ${t("check.verdict", { value: result.verdict })}`);
  }
...
export async function executeCheck({ repoRoot = process.cwd(), flags, locale = null, suppressExitCode = false } = { flags: {} }) {
  ...
  if (diffResult.fallback) {
    addFinding(store, createFinding({
      ...
      message: t("check.baseRefFallback", { baseRef, reason: diffResult.fallbackReason }),
