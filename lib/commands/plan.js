...
async function runRoughIntentPlan({ task, flags, locale, repoRoot, config, t }) {
  ...
  // 解析意图
  const parsed = parseRoughIntent(task, {
...
`);
  ...
  if (autoConfirm) {
    return saveRoughIntentContract({ task, parsed, flags, locale, repoRoot, config, t });
  }
 ...
 * 保存 Rough-Intent 契约
 */
function saveRoughIntentContract({ task, parsed, flags, locale, repoRoot, config, t }) {
  const contractPath = String(flags["contract-path"] || defaultTaskContractPath);
  const printOnly = Boolean(flags["print-only"]);
