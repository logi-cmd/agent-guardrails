...
function printProofMemoryPolicyAdvice(policyAdvice) {
  if (!policyAdvice) return;

  const mode = policyAdvice.mode || "unknown";
  console.log(`  Policy advice: ${mode}`);
  ...
  if (policyAdvice.nextAction) {
    console.log(`  Advice next: ${policyAdvice.nextAction.label || policyAdvice.nextAction.code}`);
    if (policyAdvice.nextAction.command) {
      console.log(`  Command: ${policyAdvice.nextAction.command}`);
    }
  ...
  if (policyAdvice.configPath) {
    console.log(`  Config: ${policyAdvice.configPath}`);
  }
}
...
function printProofMemoryHealth(proofMemoryHealth) {
  ...
  printProofMemoryPolicyAdvice(proofMemoryHealth.policyAdvice);
