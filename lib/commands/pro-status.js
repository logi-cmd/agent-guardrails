...
function summarizeAutomationHandoff(workbench = {}) {
  ...
  const nextCommand = automationContract.nextCommand || getNextProofCommand(workbench);
  const humanChecks = Array.isArray(automationContract.humanChecks) ? automationContract.humanChecks.filter(Boolean) : [];
  const stopConditions = Array.isArray(automationContract.stopConditions) ? automationContract.stopConditions.filter(Boolean) : [];
  const copyBrief = String(automationContract.copyBrief || "").trim();
  ...
  return {
    mode: automationDecision.state || automationContract.mode || null,
    label: automationDecision.label || null,
    badge: automationDecision.badge || null,
    summary: automationDecision.summary || automationContract.goal || null,
    humanRole: automationDecision.humanRole || null,
    nextCommand: nextCommand || null,
    rerunCommand: automationContract.rerunCommand || "agent-guardrails check --review",
    evidenceLocation: automationContract.evidenceLocation || ".agent-guardrails/evidence/",
    copyBrief,
    humanChecks,
    stopConditions
  };
...
function summarizeCurrentLoop(workbench = {}) {
  ...
  const proof = currentProofContext(workbench, action.command || getNextProofCommand(workbench));
  const command = action.command || getNextProofCommand(workbench);
  ...
  return {
    ...
    label: action.label || proof.title || null,
    description: action.description || visual.summary || proof.expectedEvidence || null,
...
function summarizeVisualVerification(workbench = {}) {
  const visual = workbench?.operatorWorkbench?.visualVerification || {};
...
function getNextProofCommand(workbench = {}) {
  return workbench?.operatorWorkbench?.automationContract?.nextCommand
  ...
  const server = createServer(async (req, res) => {
    (async () => {
      ...
      if (req.method === "GET" && url.pathname === "/") {
        ...
        const liveHtml = injectLiveWorkbenchClient(workbench.html || "<!doctype html><html><body><main><h1>Workbench unavailable</h1></main></body></html>", locale);
