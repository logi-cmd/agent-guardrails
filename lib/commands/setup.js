...
import { ensureDirectory, loadTemplate, projectRoot, readConfig, supportedAdapters, writeText } from "../utils.js";
...
function installPluginFiles(repoRoot, agent) {
  const pluginInstalls = [];
  for (const helperFile of agent.repoLocalHelperFiles) {
    if (!helperFile.includes("plugins")) continue;
    const targetPath = path.join(repoRoot, helperFile.replaceAll("/", path.sep));
    if (fs.existsSync(targetPath)) continue;
    const sourcePath = path.join(projectRoot, "lib", "daemon", "hooks", "opencode-plugin.js");
    if (!fs.existsSync(sourcePath)) continue;
    ensureDirectory(path.dirname(targetPath));
    fs.copyFileSync(sourcePath, targetPath);
    pluginInstalls.push(helperFile);
  }
  return { installed: pluginInstalls };
}
...
export async function runSetup({ positional, flags, locale = null }) {
  ...
  const pluginInstallResult = installPluginFiles(repoRoot, agent);
