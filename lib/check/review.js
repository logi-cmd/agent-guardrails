...
import { getParentScope } from "../utils.js";
...
export function suppressReviewFindings(findings) {
  ...
  const noisyStateScopes = new Set(
    findings
      ...
      .flatMap((finding) => (finding.files ?? []).map((filePath) => getParentScope(filePath)).filter(Boolean))
  );

  return findings.filter((finding) => {
    if (
      finding.code === "state-mgmt-complexity-state-file" &&
      (finding.files ?? []).some((filePath) => noisyStateScopes.has(getParentScope(filePath)))
    ) {
      return false;
    }
