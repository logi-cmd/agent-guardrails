      ...
      for (const item of jsFiles) {
        ...
        if (thenMatches && thenMatches.length >= 3) {
          addFinding(createFinding({
            ...
            message: `Nested .then() chain detected (${thenMatches.length} levels) in ${filePath}`,
            action: "建议使用 async/await 重构以提升可读性，并添加适当的错误处理",
            files: [filePath]
          }));
        }
        ...
        if ((forAwaitMatch || forEachAwaitMatch) && !fileContent.includes("Promise.all") && !fileContent.includes("Promise.allSettled")) {
          addFinding(createFinding({
            ...
            message: `await inside loop without Promise.all in ${filePath}`,
            action: "建议使用 Promise.all 或 Promise.allSettled 进行并发控制",
            files: [filePath]
          }));
        }
        ...
        if (emptyCatchMatches && emptyCatchMatches.length > 0) {
          addFinding(createFinding({
            ...
            message: `Empty .catch() detected in ${filePath}`,
            action: "建议添加错误处理逻辑，避免静默失败",
            files: [filePath]
          }));
        }
      }
  ...
  },
  {
    ...
    run({ context, addFinding, t }) {
      ...
      for (const f of changes) {
        ...
        if (filePath && /\.(png|jpg|jpeg|mp4)$/i.test(filePath) && isNewAsset) {
          const scope = (typeof getParentScope === "function") ? getParentScope(filePath) : undefined;
          addFinding(createFinding({
            severity: "warning",
            category: "performance",
          ...
          action: t("performance.perf-degradation-large-asset")
      },
      {
        message: t("performance.perf-degradation-large-change"),
          action: t("performance.perf-degradation-large-change")
      },
      {
        message: t("performance.perf-degradation-file-growth", { path: filePath }),
        action: t("performance.perf-degradation-file-growth", { path: filePath }),
      });
    }
  }
}
