...
// ---------------------------------------------------------------------------
// OS Notification — cross-platform desktop notifications
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    if (platform === "win32") {
      ...
      spawnSync("powershell", [
        ...
        `[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms') | Out-Null; [System.Windows.Forms.MessageBox]::Show('${escaped}', '${title}', 'OK', 'Warning')`
    ...
    // Notification failure must not block check flow
  }
}
...
export async function createCheckRunner(repoRoot, config, log, t, info = null) {
  let isCheckRunning = false;
  let checkTimeout = null;
  let runCount = 0;
  let consecutiveFailures = 0;
  let lastCheckHash = null;
  ...
  function scheduleCheck() {
    ...
    if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
      log(`Circuit breaker: ${consecutiveFailures} consecutive failures. Cooling down for 60s.`);
  ...
  async function runCheck() {
    ...
    // State-hash dedup: skip if same file set checked successfully last time
    try {
      const { execSync } = await import("node:child_process");
      const diffOutput = execSync("git diff --name-only", { cwd: repoRoot, encoding: "utf8", timeout: 5000 });
      const files = diffOutput.trim().split("\n").filter(Boolean).sort();
      const hash = JSON.stringify(files);
      if (hash === lastCheckHash && lastCheckOk) {
        log("Skipping check — same file set, last check passed");
        return;
      }
      lastCheckHash = hash;
    } catch {
      // If we can't compute the hash, just run the check
    }
    ...
    try {
      ...
      // OS notification for errors/warnings
      if (result?.findings?.length > 0) {
        const errorCount = result.findings.filter(f => f.severity === "error").length;
        const warnCount = result.findings.filter(f => f.severity === "warning").length;
        if (errorCount > 0) {
          sendNotification("🛡️ Guardrails: Errors found", `${errorCount} error(s) detected. Check results for details.`);
        } else if (warnCount > 0) {
          sendNotification("🛡️ Guardrails: Warnings", `${warnCount} warning(s) detected.`);
        }
      }
...
export async function run({ repoRoot, config, foreground = false, locale = null }) {
  ...
  const { scheduleCheck } = await createCheckRunner(repoRoot, config, log, t, info);
