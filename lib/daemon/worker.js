...
export async function createCheckRunner(repoRoot, config, log, t, guiServer = null) {
...
export async function run({ repoRoot, config, foreground = false, locale = null, noGui = false }) {
  ...
  const { scheduleCheck } = await createCheckRunner(repoRoot, config, log, t, guiServer);
